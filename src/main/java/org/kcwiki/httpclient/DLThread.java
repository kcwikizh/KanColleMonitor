...
import org.kcwiki.cache.Ship;
import org.kcwiki.exception.CustomedException;
import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.serializer.SerializeConfig;
import org.kcwiki.tools.swfunpacker.coredecryptor.CoreDecrypt;
import org.kcwiki.initializer.Start2DataThread;
...
import static org.kcwiki.tools.swfunpacker.coredecryptor.CoreDecrypt.shipDataList;
import org.kcwiki.tools.swfunpacker.Server;
...
import org.kcwiki.handler.thread.corePool;
import org.kcwiki.handler.thread.start2dataPool;
import static org.kcwiki.handler.thread.corePool.getTaskNum;
...
public class DLThread {
    ...
    public boolean start2() throws Exception{
        ...
        final int taskID = start2dataPool.getTaskNum();
            ...
            int failCount = 0;
            RequestConfig config = Config.makeProxyConfig(false);
                for (Map.Entry<String,String> nextAddress : org.kcwiki.cache.DBCenter.AddressList.entrySet()) {
                    ...
                    if(nextAddress.getKey().contains("/resources/image/slotitem/card")){
                        ...
                        filePath=rootFolder+File.separator+"resources"+File.separator+"image"+File.separator+"slotitem"+File.separator+fileName+"-"+org.kcwiki.cache.DBCenter.NewSlotitemDB.get(fileName).getApi_name();
                        //if  (!((new File(filePath).exists())&&( new File(filePath).isDirectory()))) { new File(filePath) .mkdirs();}
                    ...
                    try{
                        success = HttpUtils.downloadFile(nextAddress.getKey(), filePath, fileName, config);
                    } catch (CustomedException ex) {
                        failCount++;
                        if (ex.getErrorCode() == CodeDict.Exception_UnknownHost) {
                            msgPublish.msgPublisher("Start2DataThread 核心下载线程发生重大错误，已终止下载进程。",0,-1);
                            return taskID;
                        }
                    }
                ...
                msgPublish.msgPublisher("Start2新数据下载完成！下载失败："+failCount+"个文件。"+constant.LINESEPARATOR+"下载路径为："+rootFolder+File.separator+"resources",0,0);
            //new moe.kcwiki.unpackswf.RenameShipSwf().shipSwf(npath+File.separator+"resources"+File.separator+"swf"+File.separator+"ships");

            new org.kcwiki.tools.standardization.RenameShipSwf().shipSwf(rootFolder+FILESEPARATOR+"resources"+FILESEPARATOR+"swf"+FILESEPARATOR+"ships");
            return taskID;
        }   
...
//                        String opPath=rootFolder+File.separator+nextAddress.getValue();
                        ...
                        boolean success = false;
                        try{
                            success = HttpUtils.downloadFile(nextAddress.getKey(), rootFolder, nextAddress.getValue(), config);
                        } catch (CustomedException ex) {
                            if (ex.getErrorCode() == CodeDict.Exception_UnknownHost) {
                                msgPublish.msgPublisher("Start2DataThread 地图下载线程发生重大错误，已终止下载进程。",0,-1);
                                return taskID;
                            }
                        }
                        if(success){ 
                    ...
                    if(value == 0){
                        ...
                        new org.kcwiki.tools.swfunpacker.UnpackSwf().maps(decompressionFolder, rootFolder);
                ...
                if(mode==2){
                   ...
                   //MainGui.jProgressBar1.setMaximum(CoreDecrypt.shipAddressList.size());
                    for (Map.Entry<String,String> nextAddress : CoreDecrypt.shipAddressList.entrySet()) {
                        ...
                        Ship ship=org.kcwiki.cache.DBCenter.ShipDB.get(shipDataList.get(nextAddress.getKey()));
                        if(ship == null)
                            continue;
                        ...
                        String filepath=rootFolder+File.separator+ship.getApi_filename()+"-"+ship.getApi_name();
                        String filename=shipDataList.get(nextAddress.getKey())+"-"+nextAddress.getValue()+".mp3";
...
//                        int reCode=new DlCore().download(nextAddress.getKey(),opPath,folder);
                        boolean success = false;
                        try{
                            success = HttpUtils.downloadFile(nextAddress.getKey(), rootFolder+File.separator+filepath, shipDataList.get(nextAddress.getKey())+"-"+filename+".mp3", config);
                        } catch (CustomedException ex) {
                            if (ex.getErrorCode() == CodeDict.Exception_UnknownHost) {
                                msgPublish.msgPublisher("Start2DataThread 地图下载线程发生重大错误，已终止下载进程。",0,-1);
                                return taskID;
