 ...
 */
package org.kcwiki.tools.swfunpacker;

import org.kcwiki.tools.swfunpacker.comparator.src.VerifyScr;
...
public class Controller {
    ...
    private final org.kcwiki.tools.swfunpacker.comparator.image.Controller verify;
    ...
    public Controller() {
        ...
        ffdec=new UnpackSwf();
        verify=new org.kcwiki.tools.swfunpacker.comparator.image.Controller();
    }
    
    public void Analysis(String filename,String filepath,String sourcepath) {
        ...
        if(filename.contains("Core")){
            if(!new org.kcwiki.tools.swfunpacker.coredecryptor.CoreRecover().unlockCore(filepath+File.separator+filename, MainServer.getTempFolder())){msgPublish.msgPublisher("Core.swf 解码失败。",0,-1);}
            ffdec.ffdec(outputpath,MainServer.getTempFolder()+File.separator+"Core_hack.swf");
            new org.kcwiki.tools.swfunpacker.coredecryptor.CoreDecrypt().getData(outputpath+File.separator+Server.getCoremap(), outputpath+File.separator+Server.getCoresound());
